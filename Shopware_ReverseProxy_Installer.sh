...
get_reverse_proxy_ip() {
    read -p "Enter the reverse proxy IP address: " reverse_proxy_ip
}
...
install_shopware() {
    ...
    get_reverse_proxy_ip  # Ask the user for the reverse proxy IP
    ...
    <Directory /var/www/$domain_name>
        Options -Indexes +FollowSymLinks +MultiViews
        AllowOverride All
        Require ip $reverse_proxy_ip
    </Directory>

    <Location />
        Order deny,allow
        Deny from all
        Allow from $reverse_proxy_ip
    </Location>
    ...
    while true; do
        ...
        # Update or append SHOPWARE_CORE_STORE_API_URI
        if grep -q "^SHOPWARE_CORE_STORE_API_URI=" "$env_file"; then
            sudo sed -i "s|^SHOPWARE_CORE_STORE_API_URI=.*|SHOPWARE_CORE_STORE_API_URI=https://www.$domain_name/api/oauth/token|" "$env_file"
        else
            echo "SHOPWARE_CORE_STORE_API_URI=https://www.$domain_name/api/oauth/token" | sudo tee -a "$env_file" > /dev/null
        fi

        # Update or append SHOPWARE_ADMIN_API_URL
        if grep -q "^SHOPWARE_ADMIN_API_URL=" "$env_file"; then
            sudo sed -i "s|^SHOPWARE_ADMIN_API_URL=.*|SHOPWARE_ADMIN_API_URL=https://www.$domain_name/api|" "$env_file"
        else
            echo "SHOPWARE_ADMIN_API_URL=https://www.$domain_name/api" | sudo tee -a "$env_file" > /dev/null
        fi

        # Update or append APP_URL
        if grep -q "^APP_URL=" "$env_file"; then
            sudo sed -i "s|^APP_URL=.*|APP_URL=https://www.$domain_name|" "$env_file"
        else
            echo "APP_URL=https://www.$domain_name" | sudo tee -a "$env_file" > /dev/null
        fi

        # Update or append TRUSTED_PROXIES
        if grep -q "^TRUSTED_PROXIES=" "$env_file"; then
            sudo sed -i "s|^TRUSTED_PROXIES=.*|TRUSTED_PROXIES=127.0.0.1,REMOTE_ADDR|" "$env_file"
        else
            echo "TRUSTED_PROXIES=127.0.0.1,REMOTE_ADDR" | sudo tee -a "$env_file" > /dev/null
        fi
        env_local_file="/var/www/$domain_name/.env.local"
        
        # Update or append APP_URL
        if grep -q "^APP_URL=" "$env_file"; then
            sudo sed -i "s|^APP_URL=.*|APP_URL=https://www.$domain_name|" "$env_local_file"
        else
            echo "APP_URL=https://www.$domain_name" | sudo tee -a "$env_local_file" > /dev/null
        fi
        ...
        index_file="/var/www/$domain_name/public/index.php"
        sudo sed -i '/$trustedHosts = $context/i\    $_SERVER['\''HTTPS'\''] = '\''on'\'';\n    $_SERVER['\''SERVER_PORT'\''] = 443;\n    $_SERVER['\''HTTP_X_FORWARDED_PROTO'\''] = '\''https'\'';' /var/www/$domain_name/public/index.php
        htaccess_file="/var/www/$domain_name/public/.htaccess"
        echo "
        SetEnv HTTPS on
        SetEnv SERVER_PORT 443" | sudo tee -a $htaccess_file
